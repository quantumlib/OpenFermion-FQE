...
def test_first_factorization():
    ...
    for p, q, r, s in product(range(oei.shape[0]), repeat=4):
        if np.abs(tei[p, q, r, s]) < EQ_TOLERANCE:
            ...
            coefficient = tei[p, q, r, s] / 2.

        for sigma, tau in product(range(2), repeat=2):
            if 2 * p + sigma == 2 * q + tau or 2 * r + tau == 2 * s + sigma:
                continue
            term = ((2 * p + sigma, 1), (2 * q + tau, 1), (2 * r + tau, 0),
                    (2 * s + sigma, 0))
            fermion_tei += of.FermionOperator(term, coefficient=coefficient)
            test_tei_tensor[2 * p + sigma, 2 * q + tau, 2 * r + tau,
                            2 * s + sigma] = coefficient

    mol_ham = of.InteractionOperator(one_body_tensor=np.zeros((n_qubits,
                                                               n_qubits)),
...
def test_second_factorization():
    ...
    s_rho_rho, basis_changes = lrt_obj.second_factorization(eigenvalues,
                                                            one_body_squares)
    true_basis_changes = []
    for l in range(one_body_squares.shape[0]):
