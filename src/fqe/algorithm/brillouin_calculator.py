...
def get_fermion_op(coeff_tensor):
    if len(coeff_tensor.shape) == 4:
        ...
        raise ValueError("Coeff Tensor not dim 2 of 4")
...
def get_acse_residual_fqe(fqe_wf, fqe_ham, norbs):
    ...
    for p, q, r, s in product(range(norbs), repeat=4):
        ...
        if p != q and r != s:
            ...
            rdo = 1j * (of.FermionOperator(rdo) - of.hermitian_conjugated(
                of.FermionOperator(rdo)))
            val1 = fqe.util.vdot(fqe_appA, fqe_wf.apply(rdo))
            val2 = np.conjugate(
                val1)  # fqe.util.vdot(fqe_wf.apply(rdo), fqe_appA)
    ...
    acse_residual[1::2, ::2, 1::2, ::2] = np.einsum('ijkl->ijlk',
                                                    -acse_residual[1::2, ::2,
                                                     ::2, 1::2])
        ...
        if p * 2 * norbs + q >= s * 2 * norbs + r:
            if p * 2 * norbs + q != s * 2 * norbs + r:
                ...
                if abs(acse_res_tensor[p, q, r, s]) > 1.0E-12:
                    op = ((p, 1), (q, 1), (r, 0), (s, 0))
                    fop1 = of.FermionOperator(op, coefficient=acse_res_tensor[
                        p, q, r, s])
                    op = ((s, 1), (r, 1), (q, 0), (p, 0))
                    fop2 = of.FermionOperator(op, coefficient=acse_res_tensor[
                        s, r, q, p])
...
def acse_residual_atomic(p, q, r, s, fqe_appA, fqe_wf):
    ...
    return p, q, r, s, acse_aa_i, acse_aa_r, acse_bb_i, acse_bb_r, acse_ab_i, \
           acse_ab_r
...
def get_acse_residual_fqe_parallel(fqe_wf, fqe_ham, norbs):
    ...
    with Parallel(n_jobs=11, batch_size=norbs) as parallel:
        result = parallel(
            delayed(acse_residual_atomic)(p, q, r, s, fqe_appA, fqe_wf) for
            p, q, r, s in product(range(norbs), repeat=4))
...
def get_tpdm_grad_fqe_atomic(p, q, r, s, fqe_appS, fqe_wf):
    ...
    return p, q, r, s, acse_aa_r, acse_aa_i, acse_bb_r, acse_bb_i, acse_ab_r, acse_ab_i
...
def get_tpdm_grad_fqe_parallel(fqe_wf, acse_res_tensor, norbs):
    ...
    with Parallel(n_jobs=-1) as parallel:
        result = parallel(
            delayed(get_tpdm_grad_fqe_atomic)(p, q, r, s, fqe_appS, fqe_wf) for
            p, q, r, s in product(range(norbs), repeat=4))
