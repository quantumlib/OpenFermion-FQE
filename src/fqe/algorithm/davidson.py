"""
A reference implementation of Davidson-Liu diagonalization
with numpy and with FQE
"""
...
def davidsonliu(hmat: np.ndarray, nroots: int,
                guess_vecs=None, epsilon: float=1.0E-8, verbose=False):
    ...
    while len(guess_vecs) <= dim:
        ...
        # if converged return
        if verbose:
            print("eig convergence  {}, ".format(
                np.linalg.norm(w[:nroots] - old_thetas)), w[:nroots] -
                                                          old_thetas)
        if np.linalg.norm(w[:nroots] - old_thetas) < epsilon:
            ...
            for i in range(nroots):
                eigenvectors.append(
                sum([v[j, i] * guess_vecs[j] for j in range(current_num_gv)]))
        ...
        for i in range(nroots):
            ...
            subspace_eigvec_expanded = \
                sum([v[j, i] * guess_vecs[j] for j in range(current_num_gv)])

            residual = hmat @ subspace_eigvec_expanded - w[i] * \
                       subspace_eigvec_expanded
            # this is wrong. preconditioner is w[i] - np.diag(hmat)
            preconditioned_residual = np.multiply(
                residual.flatten(),
                np.reciprocal(w[i] - hmat_diag)).reshape((-1, 1))
            if verbose:
            ...
            guess_vecs.append(preconditioned_residual / np.linalg.norm(
...
def davidsonliu_fqe(hmat: Hamiltonian, nroots: int,
    ...
    # This should be changed to Slater-Condon rules construction and not
    # this hack!
    ...
    while len(guess_vecs) <= graph.lena() * graph.lenb() / 2:
        ...
        if np.linalg.norm(w[:nroots] - old_thetas) < epsilon:
            ...
            for i in range(nroots):
                eigenvectors.append(
                    sum([v[j, i] * guess_vecs[j].sector(gv_sector).coeff for j
                         in range(current_num_gv)]))
            return w[:nroots], eigenvectors
        ...
        for i in range(nroots):
            # expand in the space of all existing guess_vecs
            subspace_eigvec_expanded = \
                sum([v[j, i] * guess_vecs[j].sector(gv_sector).coeff for j in
                     range(current_num_gv)])
            subspace_eigvec = copy.deepcopy(guess_vecs[0])
            subspace_eigvec.set_wfn(strategy='from_data',
                                    raw_data={
                                        gv_sector: subspace_eigvec_expanded})
            ...
            preconditioner.set_wfn(strategy='from_data',
                                   raw_data={gv_sector: np.reciprocal(
                                       w[i] - diagonal_ham)})
            f_k_coeffs = np.multiply(preconditioner.sector(gv_sector).coeff,
                                     residual.sector(gv_sector).coeff)
            f_k = copy.deepcopy(residual)
            f_k.set_wfn(strategy='from_data',
                        raw_data={gv_sector: f_k_coeffs})

            # orthogonalize preconditioned_residual
            overlaps = []
            # print(len(guess_vecs))
            for idx in range(len(guess_vecs)):
                overlaps.append(np.sum(
                    np.multiply(guess_vecs[idx].get_coeff(gv_sector),
                                f_k.get_coeff(gv_sector))))

            for idx in range(len(guess_vecs)):
                f_k.sector(gv_sector).coeff -= overlaps[idx] * guess_vecs[
                    idx].sector(gv_sector).coeff
...
if __name__ == "__main__":
    ...
    guess_vec1_coeffs[
        graph.index_alpha(alpha_hf), graph.index_beta(beta_hf)] = 1.0
    ...
    guess_wfn1.set_wfn(strategy='from_data',
    ...
    charge = 0
    multiplicity = 1
    molecule = of.MolecularData(geometry=geometry, basis='sto-3g',
    ...
    ww, vv = davidsonliu(ham_mat + np.eye(ham_mat.shape[0]), 1,
