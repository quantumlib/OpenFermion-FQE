...
def double_factor_trotter_wf_evolution(
    ...
    for step in range(1, len(basis_change_unitaries)):
        intermediate_wfn = evolve_wf_diagonal_coulomb(
            intermediate_wfn, vij_mats[step - 1], deltat
        )
        intermediate_wfn = evolve_wf_givens(
...
def test_fqe_givens():
    ...
    assert np.allclose(
        final_fqe_wfn.rdm("i^ j^ k l"), final_wfn_test.rdm("i^ j^ k l")
    )
    ...
    assert np.allclose(
        givens_fqe_wfn.rdm("i^ j^ k l"), final_wfn_test2.rdm("i^ j^ k l")
    )
...
def test_charge_charge_evolution():
    ...
    test_wfn = evolve_wf_diagonal_coulomb(
        wf=initial_wf, vij_mat=vij, time=time
    )
    test_wfn = fqe.from_cirq(test_wfn.flatten(), 1.0e-12)

    assert np.allclose(final_fqe_wfn.rdm("i^ j"), test_wfn.rdm("i^ j"))
    assert np.allclose(
        final_fqe_wfn.rdm("i^ j^ k l"), test_wfn.rdm("i^ j^ k l")
    )
...
def test_double_factorization_trotter():
    ...
    with pytest.raises(ValueError):
        _ = double_factor_trotter_evolution(
            fqe_wfn, basis_change_unitaries + [0], vij_mats=vij_mats, deltat=0
        )
    ...
    intermediate_wfn = evolve_wf_givens(
        initial_wf.copy(), basis_change_unitaries[0]
    )
    for step in range(1, len(basis_change_unitaries)):
        intermediate_wfn = evolve_wf_diagonal_coulomb(
            intermediate_wfn, vij_mats[step - 1], time
        )
        intermediate_wfn = evolve_wf_givens(
            intermediate_wfn, basis_change_unitaries[step]
        )
    ...
    assert np.allclose(
        final_wfn.rdm("i^ j^ k l"), test_final_wfn.rdm("i^ j^ k l")
    )
