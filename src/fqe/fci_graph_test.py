...
def test_fci_graph():
    """Checks the basic initializers and getter functions."""
    reflist = [
        15,
        23,
        39,
    ...
    assert np.allclose(testgraph.string_alpha_all(), reflist)
    assert np.allclose(testgraph.string_beta_all(), [0])

    assert testgraph.index_alpha_all() == refdict
    assert testgraph.index_beta_all() == {0: 0}


def test_fci_graph_maps():
    """Checks graph mapping functions."""
    ref_alpha_map = {
    ...
    assert alpha_map == ref_alpha_map

    beta_map = testgraph._build_mapping(blist, bind)
    assert beta_map == ref_beta_map
