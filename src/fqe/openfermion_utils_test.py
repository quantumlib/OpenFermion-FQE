    ...
    def test_parse_ops_len(self):
        ...
        for term in test.terms:
            self.assertListEqual([-1, -1],
                    list(openfermion_utils.parse_one_particle_matrix_ops(term)))
        test = FermionOperator('3^ 2^ 1 0 1^ 0', 1.0)
        for term in test.terms:
            self.assertListEqual([-1, -1, -1, -1],
                    list(openfermion_utils.parse_two_particle_matrix_ops(term)))
