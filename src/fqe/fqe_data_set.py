...
class FqeDataSet:
    ...
    def _apply123(self, h1e: numpy.ndarray, h2e: numpy.ndarray, h3e: numpy.ndarray) -> 'FqeDataSet':
        """
    ...
    def apply_inplace_individual_nbody(self,
                                       coeff: complex,
                                       daga: List[int],
                                       undaga: List[int],
                                       dagb: List[int],
                        ...
                        if check:
                            ...
                            if alpha:
                                alphamap.append((index,
                                                 target._core.index_alpha(current),
                                                 (-1)**parity))
                            else:
                                betamap.append((index,
                                                target._core.index_beta(current),
                                                (-1)**parity))
    ...
    def evolve_inplace_individual_nbody(self,
                                        time: float,
                                        coeff: complex,
                                        daga: List[int],
                                        undaga: List[int],
        ...
        for key, sector in self._data.items():
            (tmp1, tmp2) = sector.apply_cos_sin(time,
                                                ncoeff,
                                                numbera + dagworka,
                                                undagworka,
                                                numberb + dagworkb,
        ...
        cosdata1.ax_plus_y(-1.0j,
                           sindata1.apply_individual_nbody(
                               numpy.conj(coeff)*phase,
                               undaga,
                               daga,
                               undagb,
                               dagb))
        cosdata2.ax_plus_y(-1.0j,
                           sindata2.apply_individual_nbody(coeff,
                                                           daga,
                                                           undaga,
                                                           dagb,
                                                           undagb))
    ...
    def rdm12(self, bra: Optional['FqeDataSet'] = None) -> Tuple[numpy.ndarray, numpy.ndarray]:
        """
    ...
    def calculate_dvec_with_coeff(self,
                    ...
                    if nalpha+1 <= norb and nbeta-1 >= 0:
                        ...
                        for sourcea, targeta, paritya in alphamap[(j, )]:
                            ...
                            for sourceb, targetb, parityb in betamap[(i, )]:
                                ...
                                out0[sourcea, sourceb] += work*paritya*parityb
                    # <0| a^+ b | dvec>
                    if nalpha-1 >= 0 and nbeta+1 <= norb:
                        dvec1 = dvec[(nalpha-1, nbeta+1)]
                        (alphamap, betamap) = sector.get_fcigraph().find_mapping(-1, 1)
