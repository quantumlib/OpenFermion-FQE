...
"""Unit tests for interfacing with Cirq."""
...
def test_pauli_x_error():
    ...
    eigenstate = np.ones(2, dtype=np.complex64)
    eigenstate[1] = 0.0 - 1.0j
    eigenstate /= np.sqrt(np.vdot(eigenstate, eigenstate))
...
def test_pauli_x():
    ...
    circuit = cirq.Circuit([cirq.Moment(gates)])
    result = qpu.simulate(circuit, qubit_order=qubit, initial_state=eigenstate)
    assert np.allclose(list(result.final_state_vector), list(eigenstate))
...
def test_build_ops_error():
    """Circuits with incorrect arguments should raise an error."""
    with pytest.raises(ValueError):
        cirq_utils.qubit_op_to_gate("W", cirq.LineQubit(0))
...
def test_build_circuit_product():
    ...
    """
    qpu = cirq.Simulator(dtype=np.complex128)
    ...
    init_state = np.zeros(2 ** 4, dtype=np.complex128)
    init_state[0] = 1.0 + 0.0j
    result = qpu.simulate(
        circuit, qubit_order=qubits, initial_state=init_state
    )
    final_state = np.zeros(2 ** 4, dtype=np.complex128)
    final_state[-1] = 1.0 + 0.0j
    assert np.allclose(list(result.final_state_vector), list(final_state))


def test_single_mode_projection():
    """Checks the coefficient of a wavefunction generated from a single qubit.
    """
    n_qubits = 1
    ...
    cof = np.zeros(n_qubits, dtype=np.complex128)
...
def test_x_y_z_mode_projection():
    ...
    Consequently, the coefficients must be set based on that iteration at
    test time.
    """
    n_qubits = 2
    qubits = cirq.LineQubit.range(n_qubits)
    test_wfn = np.array(
        [0.92377985 + 0.0j, 0.0 - 0.20947377j, 0.32054904 + 0.0j, 0.0 + 0.0j],
        dtype=np.complex128,
    )
    assert cmath.isclose(np.vdot(test_wfn, test_wfn), 1.0 + 0.0j, abs_tol=1e-6)
    ops = (
        QubitOperator("X0", 1.0)
        + QubitOperator("Y1", 1.0)
        + QubitOperator("Z0", 1.0)
    )

    test_cof = np.zeros((3, 1), dtype=np.complex128)
    ...
    cof = np.zeros((3, 1), dtype=np.complex128)
    cirq_utils.qubit_projection(ops, qubits, test_wfn, cof)
    assert np.allclose(cof, test_cof)


def test_qubit_wavefunction_from_vacuum():
    """Checks building a wavefunction given a group of qubit operations."""
    test_val = 1.0 + 2.0 + 3.0 + 5.0 + 7.0 + 11.0 + 13.0 + 0.0j
    n_qubits = 1
    qubits = cirq.LineQubit.range(n_qubits)
    ops = (
        QubitOperator("X0", 1.0)
        + QubitOperator("X0", 2.0)
        + QubitOperator("X0", 3.0)
        + QubitOperator("X0", 5.0)
        + QubitOperator("X0", 7.0)
        + QubitOperator("X0", 11.0)
